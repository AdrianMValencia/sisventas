...
(function(factory) {
        ...
        var _ = this,
            set = function(isMoveAfter) {
                if (!isMoveAfter) {
                    ...
                    _.slide.find('>*').attr('tabindex', '-1').not('.show').attr('aria-hidden', 'true').find('a,button').attr({
                        tabindex: '-1',
                        'aria-hidden': 'true'
                    });
    ...
    JdSlider.prototype.update = function() {
        var _ = this,
            ...
            width = function() {
                ...
                x = !_.opt.isLoop && _.slide.attr('data-index') >= Math.ceil((slideLen - _.opt.slideShow) / _.opt.slideToScroll) ? -slideWid * (firstIdx + slideLen - _.opt.slideShow) : -slideWid * (firstIdx + currentIdx);
                if (is.css3) {
                    _.transitionNone(_.slide);
                    _.slide.css({
            ...
            func = function() {
                ...
                if (!_.opt.isSliding || !_.opt.isUse || slideLen <= _.opt.slideShow) {
                    ...
                    _.wrap.css('width', '');
                    wrapW = window.getComputedStyle ? window.getComputedStyle(_.wrap[0], null).width.split('px')[0] : _.wrap.width();
                    margin();
                    width();
                }
        ...
        var _ = this,
            init = function() {
                var slideLen = _.slide.find('>*').not('.clone').length,
                    ...
                    firstIdx = _.slide.find('>*').not('.clone').eq(0).index(),
                    thisIdx = !_.opt.isLoop && slideIdx - firstIdx >= slideLen - _.opt.slideShow ? Math.ceil((slideLen - _.opt.slideShow) / _.opt.slideToScroll) : Math.floor((slideIdx - firstIdx) / _.opt.slideToScroll);
                _.slide.attr({
                    'data-hover': 'false',
                    'data-index': thisIdx
                }).on('click', $.proxy(_.swipe(), 'clickFn'));
                _.obj.on('click', _.opt.prev, function() {
                    this.focus();
                    _.control().prev.func();
                ...
                if (!is.mobile && _.opt.isCursor) {
                    ...
                    _.slide.on('mouseover', hover.isTrue).on('mouseout', hover.isFalse);
                    _.prev.on('mouseover', hover.isTrue).on('mouseout', hover.isFalse);
                    _.next.on('mouseover', hover.isTrue).on('mouseout', hover.isFalse);
                    _.indicate.find('a,button').on('mouseover', hover.isTrue).on('mouseout', hover.isFalse);
                    _.auto.on('mouseover', hover.isTrue).on('mouseout', hover.isFalse);
                }
                if (_.opt.isTouch) {
                    _.slide.on('touchstart', $.proxy(_.swipe(), 'touchStartFn')).on('touchmove', $.proxy(_.swipe(), 'touchMoveFn')).on('touchend', $.proxy(_.swipe(), 'touchEndFn'));
                }
                if (_.opt.isDrag) {
                    _.slide.on('mousedown', $.proxy(_.swipe(), 'dragStartFn')).on('mousemove', $.proxy(_.swipe(), 'dragMoveFn')).on('mouseup mouseleave', $.proxy(_.swipe(), 'dragEndFn'));
                }
            },
            hover = {
                isTrue: function() {
            ...
            auto = function() {
                if (!_.opt.isCustomAuto) {
                    ...
                    _.setTimer1 = setTimeout(function() {
                        _.control().interval();
                        _.setTimer2 = setInterval(_.control().interval, _.opt.interval + _.duration);
                    }, _.opt.interval);
                }
            },
            reset = function() {
                ...
                if (!is.mobile) {
                    _.slide.off('mouseover', hover.isTrue).off('mouseout', hover.isFalse);
                    _.prev.off('mouseover', hover.isTrue).off('mouseout', hover.isFalse);
                    _.next.off('mouseover', hover.isTrue).off('mouseout', hover.isFalse);
                    _.indicate.find('a,button').off('mouseover', hover.isTrue).off('mouseout', hover.isFalse);
                    _.auto.off('mouseover', hover.isTrue).off('mouseout', hover.isFalse);
                }
                _.slide.off('touchstart', $.proxy(_.swipe(), 'touchStartFn')).off('touchmove', $.proxy(_.swipe(), 'touchMoveFn')).off('touchend', $.proxy(_.swipe(), 'touchEndFn')).off('mousedown', $.proxy(_.swipe(), 'dragStartFn')).off('mousemove', $.proxy(_.swipe(), 'dragMoveFn')).off('mouseup mouseleave', $.proxy(_.swipe(), 'dragEndFn'));
            };
        return {
            init: init,
            hover: hover,
        ...
        var _ = this,
            ...
            indicate = {
                ...
                func: function(that, isTrigger) {
                    ...
                    _.currentTab().func(false);
                    if (_.opt.isSliding) {
                        this.x = _.opt.isLoop ? -this.slideWid * (this.firstIdx + this.currentIdx * _.opt.slideToScroll) : this.currentIdx === _.indicate.find('>*').length - 1 ? -this.slideWid * (this.firstIdx + this.slideLen - _.opt.slideShow) : -this.slideWid * (this.firstIdx + this.currentIdx * _.opt.slideToScroll);
                        if (is.css3) {
                            _.slide.css({
                                webkitTransform: 'translate3d(' + this.x + 'px,0,0)',
                                transform: 'translate3d(' + this.x + 'px,0,0)'
                        ...
                        if (is.css3) {
                            ...
                            _.slide.find('>.on').stop().animate({
                                opacity: 1
                            }, _.duration, _.opt.easing).siblings().animate({
                                opacity: 0
                            }, _.duration, _.opt.easing);
                        }
                        setTimeout(function() {
        ...
        var _ = this,
            ...
            touchStartFn = function(e) {
                if (!_.isMotion && _.swipes.touchStep === 0) {
                    ...
                    if (_.opt.isSliding) {
                        _.swipes.startPosition = parseFloat($.trim(_.slide.css('transform').split(',')[4]) || $.trim(_.slide.css('webkitTransform').split(',')[4]));
                    }
                    _.transitionNone(_.slide);
                    ...
                    if (!_.swipes.scrolling) {
                        if (_.opt.isSliding) {
                            ...
                            if (!_.opt.isLoop && _.opt.isResistance) {
                                ...
                                currentMove = _.swipes.position.current > 0 ? _.swipes.position.current * _.opt.resistanceRatio : _.swipes.position.current < maxDistance ? maxDistance + (_.swipes.position.current - maxDistance) * _.opt.resistanceRatio : _.swipes.position.current;
                            } else {
            ...
            dragStartFn = function(e) {
                if (!_.isMotion && _.swipes.touchStep === 0) {
                    ...
                    if (_.opt.isSliding) {
                        _.swipes.startPosition = is.ie ? (is.css3 ? parseFloat($.trim(_.slide.css('transform').split(',')[12])) : parseFloat(_.slide.css('marginLeft').split('px')[0])) : (parseFloat($.trim(_.slide.css('transform').split(',')[4]) || $.trim(_.slide.css('webkitTransform').split(',')[4])));
                    }
                    _.transitionNone(_.slide);
                    _.swipes.touchStep = 1;
                }
    ...
    JdSlider.prototype.trigger = function() {
        var _ = this,
            event = function() {
                _.obj.on('init slideInit', $.proxy(_.init(), 'func')) // 구조 초기화
            ...
            reset = function() {
                _.obj.off('init slideInit', $.proxy(_.init(), 'func')).off('update', $.proxy(_.update(), 'func')).off('resizeFn', $.proxy(_.resize(), 'func')).off('removeFn', $.proxy(_.remove(), 'func'));
